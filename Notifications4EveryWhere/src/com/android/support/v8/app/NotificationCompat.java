...
public class NotificationCompat {
	 ...
	 * less important. The UI may choose to show these items smaller, or at a
	 * different position in the list, compared with your app's
	 ...
	 * notifications or alerts. The UI may choose to show these items larger, or
	 * at a different position in notification lists, compared with your app's
	 * {@link #PRIORITY_DEFAULT} items.
	 */
		...
		public Notification build(Builder b) {
			...
			if (result.number != 0) {
				...
				contentView.setTextViewText(R.id.info,
						f.format(b.mNotification.number));
			}

			if (b.mProgressMax != 0 || b.mProgressIndeterminate) {
				contentView.setProgressBar(R.id.progress, b.mProgressMax,
						b.mProgress, b.mProgressIndeterminate);
				if(!isFROYO){
	...
	static class NotificationCompatImplHoneycomb implements
			NotificationCompatImpl {
		public Notification build(Builder b) {
			return NotificationCompatHoneycomb.add(b.mContext, b.mNotification,
					b.mContentTitle, b.mContentText, b.mContentInfo,
					b.mTickerView, b.mNumber, b.mContentIntent,
					b.mFullScreenIntent, b.mLargeIcon);
		}
	}

	static class NotificationCompatImplIceCreamSandwich implements
			NotificationCompatImpl {
		public Notification build(Builder b) {
			return NotificationCompatIceCreamSandwich.add(b.mContext,
					b.mNotification, b.mContentTitle, b.mContentText,
					b.mContentInfo, b.mTickerView, b.mNumber, b.mContentIntent,
					b.mFullScreenIntent, b.mLargeIcon, b.mProgressMax,
					b.mProgress, b.mProgressIndeterminate);
		}
	}

	static class NotificationCompatImplJellybean implements
			NotificationCompatImpl {
		public Notification build(Builder b) {
			NotificationCompatJellybean jbBuilder = new NotificationCompatJellybean(
					b.mContext, b.mNotification, b.mContentTitle,
					b.mContentText, b.mContentInfo, b.mTickerView, b.mNumber,
					b.mContentIntent, b.mFullScreenIntent, b.mLargeIcon,
					b.mProgressMax, b.mProgress, b.mProgressIndeterminate,
					b.mUseChronometer, b.mPriority, b.mSubText);
			for (Action action : b.mActions) {
				jbBuilder.addAction(action.icon, action.title,
						action.actionIntent);
			}
			if (b.mStyle != null) {
				if (b.mStyle instanceof BigTextStyle) {
					...
					jbBuilder.addBigTextStyle(style.mBigContentTitle,
							style.mSummaryTextSet, style.mSummaryText,
							style.mBigText);
	...
	/**
	 * Builder class for {@link Notification} objects. Allows easier control
	...
	public static class Builder {
		 ...
		 * Constructor.
		 * 
		 * Automatically sets the when field to
		 ...
		 * 
		 * Instead of presenting <code>when</code> as a timestamp, the
		 * notification will show an automatically updating display of the
		 * minutes and seconds since <code>when</code>.
		 * 
		 ...
		 * 
		 * @param icon
		 *            A resource ID in the application's package of the drawble
		 *            to use.
		 */
		...
		/**
		 * A variant of {@link #setSmallIcon(int) setSmallIcon(int)} that takes
		...
		/**
		 * Set the large number at the right-hand side of the notification. This
		...
		/**
		 * Supply a {@link PendingIntent} to send when the notification is
		 * clicked. If you do not supply an intent, you can now add
		 * PendingIntents to individual views to be launched when clicked by
		 * calling {@link RemoteViews#setOnClickPendingIntent
		...
		/**
		 * Supply a {@link PendingIntent} to send when the notification is
		 * cleared by the user directly from the notification panel. For
		 * example, this intent is sent when the user clicks the "Clear all"
		 * button, or the individual "X" buttons on notifications. This intent
		 ...
		 * bar. Only for use with extremely high-priority notifications
		 * demanding the user's <strong>immediate</strong> attention, such as an
		 * incoming phone call or alarm clock that the user has explicitly set
		 ...
		 * @param highPriority
		 *            Passing true will cause this notification to be sent even
		...
		public Builder setLights(int argb, int onMs, int offMs) {
			...
			boolean showLights = mNotification.ledOnMS != 0
		 ...
		 * 
		 * <p>
		 * Ongoing notifications differ from regular notifications in the
		 * following ways:
		 * <ul>
		 * <li>Ongoing notifications are sorted above the regular notifications
		 * in the notification panel.</li>
		 * <li>Ongoing notifications do not have an 'X' close button, and are
		 ...
		 * canceled when the user clicks it in the panel. The PendingIntent set
		 ...
		 * bitwise-or: {@link Notification#DEFAULT_SOUND},
		 ...
		 * 
		 * Priority is an indication of how much of the user's valuable
		 ...
		 * notification. The system will make a determination about how to
		 * interpret notification priority as described in MUMBLE MUMBLE.
		 */
	 ...
	 * 
	 * This class is a "rebuilder": It attaches to a Builder object and modifies
	 * its behavior, like so:
	 * 
	 ...
	 * 		.setLargeIcon(aBitmap)
	 * 		.setStyle(
	 * 				new Notification.InboxStyle().addLine(str1).addLine(str2)
	 * 						.setContentTitle(&quot;&quot;).setSummaryText(&quot;+3 more&quot;)).build();
	 * </pre>
	 * 
	 * @see Notification#bigContentView
	 */
